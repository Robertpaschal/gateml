...
export function DashboardPage() {
  ...
  useEffect(() => {
    if (!user) return;
    statsApi.get(user.token).then(setStats).catch(console.error);
    logsApi.list(user.token, 6).then(setLogs).catch(console.error);
  }, [user]);
  ...
  return (
    <div className="fade-in">
      ...
      <div className="grid-4">
        ...
        <StatCard
          label="Spend Today"
          value={stats ? `$${stats.totalCostUsd.toFixed(2)}` : "—"}
          color="orange"
        />
        <StatCard
          label="p95 Latency"
          value={stats ? `${stats.p95LatencyMs.toLocaleString()}ms` : "—"}
          color="yellow"
        />
      </div>
          ...
          <div className="card">
            <div className="card-header">
              <div>
                ...
                <div className="card-sub">Hourly, last 24 h</div>
              </div>
      ...
      <div className="card">
        ...
        {logs.length === 0 ? (
          <div style={{ color: "var(--muted)", fontSize: 12, padding: "20px 0" }}>
            No requests yet. Make your first call through the Gateway and it'll appear here.
          </div>
        ) : (
          logs.map(l => (
