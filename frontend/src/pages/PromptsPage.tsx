...
const PROMPTS = [
  { id: "p1", name: "Customer Support Bot",  status: "production" as const, version: 4, model: "gpt-4o" },
  { id: "p2", name: "Document Summarizer",   status: "staging"    as const, version: 2, model: "claude-sonnet-4-6" },
  { id: "p3", name: "Code Review Assistant", status: "production" as const, version: 7, model: "gpt-4o-mini" },
];
          ...
          {PROMPTS.map(p => (
            <div key={p.id} onClick={() => setSelected(p.id)} style={{
              ...
              <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                <span className={`tag tag-${p.status === "production" ? "green" : p.status === "staging" ? "yellow" : "gray"}`} style={{ fontSize: 9 }}>{p.status}</span>
        ...
        <div style={{ flex: 1 }}>
          <div className="card" style={{ marginBottom: 16 }}>
            ...
            <div style={{ marginBottom: 16 }}>
              {VERSIONS.map(v => (
                <div key={v.v} onClick={() => setActiveV(v.v)} style={{
                  display: "flex", alignItems: "center", gap: 12, padding: "10px 12px", borderRadius: 4,
                  cursor: "pointer", background: activeV === v.v ? "var(--surface2)" : "transparent", marginBottom: 4, transition: "all 0.12s",
                }}>
                  <div style={{ width: 28, height: 28, borderRadius: 4, display: "flex", alignItems: "center", justifyContent: "center", background: v.isActive ? "rgba(0,255,136,0.15)" : "var(--surface2)", color: v.isActive ? "var(--accent)" : "var(--muted)", fontSize: 11, fontWeight: 700 }}>v{v.v}</div>
                  <div style={{ flex: 1 }}>
            ...
            {showDiff ? (
              ...
              <div>
                <div style={{ fontSize: 10, color: "var(--muted)", marginBottom: 8, letterSpacing: "1px", textTransform: "uppercase" }}>Prompt body · v{activeV}</div>
                <div className="code-area" style={{ fontSize: 11 }}>{activeV === 4 ? BODY_V4 : BODY_V3}</div>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="modal-backdrop" onClick={() => setShowNew(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              <textarea className="code-input" rows={10} value={newBody} onChange={e => setNewBody(e.target.value)} />
            </div>
            <div>
              <label className="form-label">Change Note</label>
              <input className="field" placeholder="e.g. Improved tone" value={newNote} onChange={e => setNewNote(e.target.value)} />
            </div>
